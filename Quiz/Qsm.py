...
class Qsm:
    ...
    def __init__(self) -> None:
        ...
        self.op_escolhido = IntVar()
        self.opcao = self.radio_buttons()
        self.mostrar_opcoes()
        self.buttons()
        self.tm_dados = len(pergunta)
        self.correto = 0

    def mostrar_resultado(self):
        cont_err = self.tm_dados - self.correto
        correto = f'Acertos: {self.correto}'
        erros = f'Erros: {cont_err}'

        score = int(self.correto/ self.tm_dados * 100)
    ...
    def ver_pergunta(self, pergunta_no) -> bool:
        ...
        if self.op_escolhido.get() == resposta[pergunta_no]:
            return True
